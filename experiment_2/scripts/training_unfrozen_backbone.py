...
def get_model(num_classes):
    ...
    # CRITICAL FIX: Unfreeze the last ResNet stage (blocks[5]) 
    # This allows the model to learn football-specific spatial-temporal features
    for param in model.blocks[5].parameters():
        param.requires_grad = True

    # Swap out head projection layer (which is initialized with requires_grad=True automatically)
...
sweep_config = {
    'method': 'grid',
    'metric': {
        'name': 'val_f1_score',  # Sweep now optimizes for Macro F1 to handle imbalanced classes properly
        'goal': 'maximize'
    },
    'parameters': {
        'batch_size': {'value': 4},  # Reduced to 4 because unfrozen blocks consume massive GPU memory for gradients
        'learning_rate': {'value': 0.00005}, # Lowered LR for fine-tuning pre-trained weights safely

        # FIXED: Increased to 25 to let your classification head learn rare patterns thoroughly
        'epochs': {'value': 25},

        "architecture": {'value': "SlowFast_Unfrozen_Block5"},
        'loss_type': {'values': ["focal_loss"]},  # Removed cross_entropy as focal_loss is vastly superior here
        'action_weight_multiplier': {'value': 4.0},
        'focal_gamma': {'values': [2.0]}, # Increased gamma to 2.0 to push focus more on hard rare classes
        "dataset": {'value': "SoccerNet-v2"}
    }
}

sweep_id = wandb.sweep(sweep_config, project="slowfast_unfrozen_backbone")
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, num_epochs, criterion, device):
    ...
    for epoch in range(num_epochs):
        ...
        # Save model checkpoint weight into organised folder
...
def train():
    with wandb.init() as run:
        config = wandb.config
        wandb.run.name = f"unfrozen_bs{config.batch_size}_lr{config.learning_rate}_{config.loss_type}"
